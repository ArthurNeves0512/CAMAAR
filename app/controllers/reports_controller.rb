...
class ReportsController < ApplicationController
  ...
  def export_to_csv
    ...
    questionnaire = Questionnaire.find(session[:current_user_id])  # Corrigi o 'sessions[current_user_id]' para 'session[:current_user_id]'
  ...
  def generate_csv(questionnaire)
    CSV.generate(headers: true) do |csv|
      ...
      questionnaire.answers.each do |answer|
        csv << [answer.id, answer.question.text, answer.value]
      end
    end
  end
end
